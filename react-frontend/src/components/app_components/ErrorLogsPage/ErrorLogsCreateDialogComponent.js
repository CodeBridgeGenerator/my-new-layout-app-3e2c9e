...
const ErrorLogsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            serviceName: _entity?.serviceName,errorMessage: _entity?.errorMessage,message: _entity?.message,stack: _entity?.stack,details: _entity?.details,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Error Logs created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Error Logs" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Error Logs" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
